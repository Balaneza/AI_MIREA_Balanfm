...
class QualityFlagsResponse(BaseModel):
    """Вывод флагов качества датасета."""

    too_few_rows: bool = Field(
        ...,
        description="Слишком мало строк",
    )
    too_many_missing: bool = Field(
        ...,
        description="Слишком много пропусков",
    )
    has_constant_columns: bool = Field(
        ...,
        description="Имеет повторяющиеся столбцы",
    )
    has_many_zero_values: bool = Field(
        ...,
        description="Слишком много нулевых значений",
    )
    max_missing_share: float = Field(
        description="Макс. доля пропусков по колонке:",
    )
    quality_score: float = Field(
...
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    ...
    Эндпоинт, который принимает CSV-файл, запускает EDA-ядро
    (summarize_dataset + missing_table + compute_quality_flags)
    и возвращает флаги оценки качества данных в формате json.

    Именно это по сути связывает наши дополнения в S03 (CLI EDA) и S04 (HTTP-сервис).
    """
    ...
    # Ожидаем, что compute_quality_flags вернёт quality_score в [0,1]
    too_few_rows = bool(flags_all.get("too_few_rows"))
    too_many_missing = bool(flags_all.get("too_many_missing"))
    has_constant_columns = bool(flags_all.get("has_constant_columns"))
    has_many_zero_values = bool(flags_all.get("has_many_zero_values"))
    quality_score = float(flags_all.get("quality_score",0.0))
    max_missing_share = float(flags_all.get("max_missing_share",0.0))

    print(
        f"[qualityflags-from-csv] filename={file.filename!r} "
    )
